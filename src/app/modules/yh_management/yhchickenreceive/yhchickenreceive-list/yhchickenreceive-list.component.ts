...
@Component({
    selector: 'yhchickenreceive',
    templateUrl: './yhchickenreceive-list.component.html',
    styleUrls: ['./yhchickenreceive-list.component.scss'],
})
export class YhChickenReceiveListComponent {
    datasource: any = null;
    ...
    constructor(
        ...
        this.excelModel = {
            title: this.importService.chickenreceive.title,
            xlsxTemplatePath: this.importService.chickenreceive.xlsxPath,
            jsonTemplatePath: this.importService.chickenreceive.jsonPath,
            importServer: this.importService.chickenreceive.server,
    ...
    //#region 初始化表格配置
    init_data_grid(): YhChickenReceiveListComponent {
        this.dataGridModel.primaryKey = 'NumericalOrder';
        const now = new Date();
        this.dataGridModel.export.fileName = `${
            this.translator.I18N.YhChickenReceive.title
        ...
        this.dataGridModel.stateStoring.storageKey = 'YhChickenReceive-list';
    ...
    get columns() {
        ...
        const col_dataDate = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.DataDate.text,
            'DataDate',
            'date',
            'DataDate'
        );
        ...
        const col_YHFarmerID = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.YHFarmerID.text,
            'YHFarmerName',
            'string',
        );
        ...
        const col_YHBatch = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.YHBatch.text,
            'YHBatchName',
            'string'
        );
        ...
        const col_Abstract = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.Abstract.text,
            'AbstractName',
            'string',
        );
        col_Abstract.props.allowHeaderFiltering = false;
        col_Abstract.props.lookup.enabled = true;
        col_Abstract.props.alignment = 'left'
        ...
        const col_BreedingName = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.BreedName.text,
            'BreedingName',
            'string',
        );
        col_BreedingName.props.filterOperations = ['contains',"="];
        col_BreedingName.props.allowHeaderFiltering = false;
        col_BreedingName.props.alignment = 'left'

        const col_ChickenType = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.ChickenType.text,
            'ChickenTypeName',
            'string',
        );
        col_ChickenType.props.filterOperations = ['contains',"="];
        col_ChickenType.props.allowHeaderFiltering = false;
        col_ChickenType.props.alignment = 'center';

        const col_OutWarehouse = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.OutWarehouse.text,
            'OutWarehouseName',
            'number',
        );
        col_OutWarehouse.props.filterOperations = ['between', '=','<>','<','<=','>','>='];
        col_OutWarehouse.props.selectedFilterOperation = 'between';
        col_OutWarehouse.props.allowHeaderFiltering = false;
        col_OutWarehouse.props.alignment = 'left'

        const col_InWarehouse = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.InWarehouse.text,
            'InWarehouseName',
            'string',
        );
        col_InWarehouse.props.filterOperations = ['contains',"="];
        col_InWarehouse.props.allowHeaderFiltering = false;
        col_InWarehouse.props.alignment = 'left'


        const col_Driver = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.Driver.text,
            'DriverName',
            'string',
        );
        col_Driver.props.allowHeaderFiltering = false;
        col_Driver.props.lookup.enabled = true;
        col_Driver.props.alignment = 'center'

        const col_SupplierName = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.SupplierID.text,
            'SupplierID',
            'string'
        );
        col_SupplierName.props.allowHeaderFiltering = false;
        col_SupplierName.props.lookup.enabled = true;
        col_SupplierName.props.allowHeaderFiltering = false;
        col_SupplierName.props.lookup.dataSource =  this.qlwCustomerContext.getSupplierDataSource({
            filter: [['IsUse', '=', true]],
        });
        col_SupplierName.props.lookup.valueExpr = 'SupplierId';
        col_SupplierName.props.lookup.displayExpr = 'SupplierName';

        const col_SourceType = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.SourceType.text,
            'SourceType',
            'string'
        );
        col_SourceType.props.allowHeaderFiltering = false;
        col_SourceType.props.lookup.enabled = true;
        col_SourceType.props.allowHeaderFiltering = false;
        col_SourceType.props.lookup.dataSource =  this.basicSettingODataContext.getBizDataDictDataSource({
            filter: [['Pid', '=', "2202371543490000055"]],
            select: ['DictId','DictName']
        });
        col_SourceType.props.lookup.valueExpr = 'DictId';
        col_SourceType.props.lookup.displayExpr = 'DictName';
        ...
        const col_ChickenFarmID = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.ChickenFarmID.text,
            'ChickenFarmName',
            'string'
        );
        ...
        const col_TotalQuantity = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.TotalQuantity.text,
            'TotalQuantity',
            'number',
        );
        col_TotalQuantity.props.filterOperations = ['between', '=','<>','<','<=','>','>='];
        col_TotalQuantity.props.selectedFilterOperation = 'between';
        ...
        const col_ConfirmStatus = new NxDataGridColumn(
            this.translator.I18N.YhChickenReceive.ConfirmStatus.text,
            'ConfirmStatus',
            'boolean',
        );
    ...
    edit(rowData) {
        this.listInstance.yheditToDetail(
            '/yhchickenreceive/detail',
            rowData.rowIndex,
            this.translator.I18N.YhChickenReceive.editPageTitle,
            {
    ...
    //#endregion
    //#region 初始化工具条配置
    init_toolbar_panel(): YhChickenReceiveListComponent {
        ...
        this.toolbarPanelModel.settings.push(
            ...[
                ...
                new ColumnSetting(this.translator.I18N.YhChickenReceive.DataDate.text, 'DataDate'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.YHFarmerID.text, 'YHFarmerID'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.YHBatch.text, 'YHBatch'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.ChickenFarmID.text, 'ChickenFarmID'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.Abstract.text, 'Abstract'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.OutWarehouse.text, 'OutWarehouse'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.InWarehouse.text, 'InWarehouse'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.BreedName.text, 'BreedingName'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.ChickenType.text, 'ChickenType'),
                new ColumnSetting(this.translator.I18N.YhChickenReceiveDetail.ProductName.text, 'ProductID'),
                new ColumnSetting(this.translator.I18N.YhChickenReceiveDetail.ValueQuantity.text, 'ValueQuantity'),
                new ColumnSetting(this.translator.I18N.YhChickenReceiveDetail.AmountTotal.text, 'AmountTotal'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.TotalQuantity.text, 'TotalQuantity'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.Driver.text, 'Driver'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.SourceType.text, 'SourceType'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.SupplierID.text, 'SupplierID'),
                new ColumnSetting(this.translator.I18N.YhChickenReceive.ConfirmStatus.text, 'ConfirmStatus'),
        ...
        // 设置隐藏列缓存
        this.toolbarPanelModel.storageKey = 'YhChickenReceive-columns-storage';
    ...
    create() {
        this.listInstance.yhcreateToDetail(
            '/yhchickenreceive/detail',
            this.translator.I18N.YhChickenReceive.createPageTitle
        );
    }
    rangeDelete() {
        const deleteKeys = [];
    ...
    init_search_panel() {
        ...
        date.label = this.translator.I18N.YhChickenReceive.DataDate.text;
        ...
        // condition_SupplierId.label = this.translator.I18N.YhChickenReceive.SupplierID.text;
        ...
        condition_YHFarmerID.label = this.translator.I18N.YhChickenReceive.YHFarmerID.text;
