...
import { MessageBox } from 'src/app/providers/notify';
...
import { PermissionContext, YHBasicSettingODataContext} from 'src/app/providers/odataContext';
...
import { FarmingBatchSubsidyService } from '../farmingbatchsubsidy.service';
import { NxTextBox } from 'src/app/components/component-model/text-box/mode';

@Component({
    selector: 'app-farmingbatchsubsidy-list',
    templateUrl: './farmingbatchsubsidy-list.component.html',
    styleUrls: ['./farmingbatchsubsidy-list.component.scss'],
    providers: [FarmingBatchSubsidyService],
})
export class FarmingBatchSubsidyListComponent {
    datasource: any = null;
    ...
    constructor(
        ...
        private service: FarmingBatchSubsidyService,
    ...
    //#region 初始化表格配置
    init_data_grid(): FarmingBatchSubsidyListComponent {
        ...
        this.dataGridModel.stateStoring.storageKey = 'farmingbatchsubsidy-list';
        return this;
    }
    get columns() {
        ...
        const col_SunsidyAmount = new NxDataGridColumn(
            "补扣金额",
            'SunsidyAmount',
            'number',
        );
        col_SunsidyAmount.props.filterOperations = ['between', '=','<>','<','<=','>','>='];
        col_SunsidyAmount.props.selectedFilterOperation = 'between';
        col_SunsidyAmount.props.allowHeaderFiltering = false;
        col_SunsidyAmount.props.alignment = 'right'

        const col_AdjustAmount = new NxDataGridColumn(
            "调整后金额",
            'AdjustAmount',
            'number',
        );
        col_AdjustAmount.props.filterOperations = ['between', '=','<>','<','<=','>','>='];
        col_AdjustAmount.props.selectedFilterOperation = 'between';
        col_AdjustAmount.props.allowHeaderFiltering = false;
        col_AdjustAmount.props.alignment = 'right'
    ...
    create() {
        HomeHelper.open(
            {
                title: '养殖批次补扣-新增',
                url: `${environment.qqlwUri}/farmingbatchsubsidy/detail?mode=create`,
            },
            () => {
    ...
    edit(rowData) {
        HomeHelper.open(
            {
                title: '养殖批次补扣-编辑',
                url: `${environment.qqlwUri}/farmingbatchsubsidy/detail?mode=edit&numericalOrder=${rowData.data.NumericalOrder}`,
            },
            () => {
                this._router.navigate(['/farmingbatchsubsidy/detail'], {
                    queryParams: {
    ...
    //#endregion
    //#region 初始化工具条配置
    init_toolbar_panel(): FarmingBatchSubsidyListComponent {
        ...
        this.toolbarPanelModel.settings.push(
            ...[
                ...
                new ColumnSetting("养户",'YHFarmerName'),
                new ColumnSetting("批次",'YHBatchName'),
                new ColumnSetting("养殖场",'ChickenFarmName'),
                new ColumnSetting("补扣金额",'SunsidyAmount'),
                new ColumnSetting("调整后金额",'AdjustAmount'),
        ...
        // 设置隐藏列缓存
        this.toolbarPanelModel.storageKey = 'farmingbatchsubsidy-columns-storage';
