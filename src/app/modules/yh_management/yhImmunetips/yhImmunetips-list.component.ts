...
import { yhImmunetipsService } from './yhImmunetips.service';
...
@Component({
    selector: 'app-yhImmunetips-list',
    templateUrl: './yhImmunetips-list.component.html',
    styleUrls: ['./yhImmunetips-list.component.scss'],
})
export class yhImmunetipsListComponent {
    datasource: any = null;
    StateTypeSource: any;
    YHFarmerSource: any;
    YHBatchSource: any;
    ...
    ImmuneSubjectSource: any;
    constructor(
        private service: yhImmunetipsService,
        ...
        this.basicSettingODataContext.Product.load().then((res:any) => {
            let data = res.filter(o => o.iSortPlus == DataDictionary.iSortC );
            this.CommonNameSource = Distinct(data,'CommonName');
        })
        //批次
        this.YHBatchSource = this.yhBasicSettingODataContext.getYHBatchDataSource();
        //免疫项目
        this.ImmuneSubjectSource = this.basicSettingODataContext.getImmuneSubjectDataSource({
            select: ['ImmuneSubjectID', 'ImmuneSubjectName']
        });
        // 客户
        ...
        //免疫程序
        this.service.ZqImmuneProcess('').then((res:any) => {
            this.ProcessSource = res.value;
        })
    ...
    querySearch() {
        ...
        if (data.ImmuneSubjectID) {
            filter.push(['ImmuneSubjectID', '=', data['ImmuneSubjectID']]);
        }
