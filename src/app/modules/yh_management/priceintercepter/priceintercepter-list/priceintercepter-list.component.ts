...
import { QlwODataContext,BasicSettingODataContext, PermissionContext, YHBasicSettingODataContext} from 'src/app/providers/odataContext';
...
import { PriceIntercepterService } from '../priceintercepter.service';

@Component({
    selector: 'app-priceintercepter-list',
    templateUrl: './priceintercepter-list.component.html',
    styleUrls: ['./priceintercepter-list.component.scss'],
    providers: [PriceIntercepterService],
})
export class PriceIntercepterListComponent {
    datasource: any = null;
    ...
    constructor(
        ...
        private service: PriceIntercepterService,
    ...
    //#region 初始化表格配置
    init_data_grid(): PriceIntercepterListComponent {
        ...
        this.dataGridModel.stateStoring.storageKey = 'priceintercepter-list';
        return this;
    }
    get columns() {
        ...
        const col_ProductCollectType = new NxDataGridColumn(
            '商品类型',
            'ProductCollectType',
            'string',
        );
        col_ProductCollectType.props.allowHeaderFiltering = false;
        col_ProductCollectType.props.lookup.enabled = true;
        col_ProductCollectType.props.lookup.dataSource = this.BasicSettingODataContext.getBizDataDictDataSource({
            filter: [['Pid', '=', DataDictionary.ProductCollectType]],
            select: ['DictId','DictName']
        });
        col_ProductCollectType.props.lookup.valueExpr = 'DictId';
        col_ProductCollectType.props.lookup.displayExpr = 'DictName';
        ...
        col_EndDate.props.allowFiltering = false;
        // col_EndDate.props.filterOperations = ['between', '='];
        // col_EndDate.props.selectedFilterOperation = 'between';
        col_EndDate.props.calculateDisplayValue = (row) => {
            if (row.EndDate) {
                return new DateTime(row.EndDate).toString('yyyy-MM-dd');
            } else {
                return '';
            }
        };
        ...
        const col_YHFarmerName = new NxDataGridColumn(
            '养户名称',
            'YHFarmerName',
            'string',
        );
        col_YHFarmerName.props.filterOperations = ['contains',"="];
        col_YHFarmerName.props.allowHeaderFiltering = false;

        return [
            col_dataDate,
            col_number,
            col_ProductCollectType,
            col_EffectDate,
            col_EndDate,
            col_remarks,
            col_YHFarmerName,
    ...
    create() {
        HomeHelper.open(
            {
                title: '领用特殊价格-新增',
                url: `${environment.qqlwUri}/priceintercepter/detail?mode=create`,
            },
            () => {
    ...
    edit(rowData) {
        HomeHelper.open(
            {
                title: '领用特殊价格-编辑',
                url: `${environment.qqlwUri}/priceintercepter/detail?mode=edit&numericalOrder=${rowData.data.NumericalOrder}`,
            },
            () => {
                this._router.navigate(['/priceintercepter/detail'], {
                    queryParams: {
    ...
    //#endregion
    //#region 初始化工具条配置
    init_toolbar_panel(): PriceIntercepterListComponent {
        ...
        // 设置隐藏列缓存
        this.toolbarPanelModel.storageKey = 'priceintercepter-columns-storage';
    ...
    init_search_panel() {
        ...
        const condition_ProductCollectType = new NxConditionItem();
        condition_ProductCollectType.label = "商品类型";
        condition_ProductCollectType.type = 'SelectBox';
        condition_ProductCollectType.dataField = 'ProductCollectType';
        condition_ProductCollectType.widget = new NxSelectBox();
        condition_ProductCollectType.widget.props.showClearButton = true;
        condition_ProductCollectType.widget.props.disabled = false;
        condition_ProductCollectType.widget.props.valueExpr = 'DictId';
        condition_ProductCollectType.widget.props.displayExpr = 'DictName';
        condition_ProductCollectType.widget.props.dataSource = this.BasicSettingODataContext.getBizDataDictDataSource({
        ...
        let EndDate = new NxConditionItem();
        EndDate.label = '截止日期';
        EndDate.type = 'StartEndDateBox';
        EndDate.dataField = 'EndDate';
        EndDate.widget = new NxDateBox();
        EndDate.widget.props.showClearButton = true;
        EndDate.widget.props.max = new Date();

        const condition_YHFarmerID = new NxConditionItem();
        condition_YHFarmerID.label = '养户名称';
    ...
    search(data) {
        ...
        if (data['ProductCollectType']) {
            filter.push(['ProductCollectType', '=', data['ProductCollectType']]);
        }
        ...
        if (data.EndDate&&data.EndDate[0] && data.EndDate[1]) {
            filter.push([
                ['EndDate', '>=', new DateTime(data.EndDate[0]).toString('yyyy-MM-dd')],
                'and',
                [
                    'EndDate',
                    '<=',
                    new DateTime(data.EndDate[1]).toString('yyyy-MM-dd'),
                ],
            ]);
        }
        else {
            if (data.EndDate&&data.EndDate[0]) {
                filter.push(['EndDate', '>=', new DateTime(data.EndDate[0]).toString('yyyy-MM-dd')]);
            }
        ...
        if (data['YHFarmerID']) {
            filter.push([['YHFarmerID', 'contains', data['YHFarmerID']]]);
        }
